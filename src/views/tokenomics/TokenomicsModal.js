...
const TokenomicsModal = ({
  ...
  const [percentageCirculatingSupply, setPercentageCirculatingSupply] =
    useState("");
  const [maxSupply, setMaxSupply] = useState("");
  const [supplyModel, setSupplyModel] = useState("");

  // States for responsemessages
  const [responseMessageTokenomics, setResponseMessageTokenomics] = useState({
    success: "",
    error: "",
  });
  const [responseMessageTokenUtility, setResponseMessageTokenUtility] =
    useState({ success: "", error: "" });
  const [responseMessageTokenDistri, setResponseMessageTokenDistri] = useState({
    success: "",
    error: "",
  });
  const [responseMessageValueAccrual, setResponseMessageValueAccrual] =
    useState({ success: "", error: "" });
  ...
  const handleSubmitTokenUtility = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        setTokenApplication("");
        setTokenDescription("");
        setResponseMessageTokenUtility({
          ...responseMessageTokenUtility,
  ...
  const handleSubmitTokenomics = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMaxSupply("");
        setMaxSupply("");
        setResponseMessageTokenomics({
          ...responseMessageTokenomics,
          success: data.message,
          error: "",
        });
        return data.message;
  ...
  const handleSubmitTokenDistribution = async (event) => {
    ...
    try {
      const response = await fetch(
        `${config.BASE_URL}/post_token_distribution`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        setHolderCategory("");
        setPercentageHeld("");
        setResponseMessageTokenDistri({
          ...responseMessageTokenDistri,
          success: data.message,
          error: "",
        });
        return data.message;
  ...
  const handleSubmitValueAccrualMechanisms = async (event) => {
    ...
    try {
      const response = await fetch(
        `${config.BASE_URL}/post_value_accrual_mechanisms`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        setResponseMessageValueAccrual({
          ...responseMessageValueAccrual,
          success: data.message,
          error: "",
        });
        return data.message;
    ...
    <Modal show={showModal} onHide={handleClose}>
      ...
      <Modal.Body>
        ...
        <Form className="modalForm" onSubmit={handleSubmitTokenomics}>
          ...
          />
          <Form.Group className="customInputMain" controlId="supplyModel">
            <Form.Label className="customInputLanel" >Supply Model</Form.Label>
            <Form.Control
              as="select"
              value={supplyModel}
              onChange={(e) => handleSupplyModelChange(e.target.value)}
            >
              <option value="">Select Supply Model</option>
          ...
          <div>
            ...
            {responseMessageTokenomics.success && (
              <Alert className="ModalAlert" variant="success">
                {responseMessageTokenomics.success}
              </Alert>
            )}
            {responseMessageTokenomics.error && (
              <Alert className="ModalAlert" variant="danger">
                {responseMessageTokenomics.error}
              </Alert>
            )}
          </div>
        ...
        <Form className="modalForm" onSubmit={handleSubmitTokenUtility}>
          ...
          <div>
            ...
            {responseMessageTokenUtility.success && (
              <Alert className="ModalAlert" variant="success">
                {responseMessageTokenUtility.success}
              </Alert>
            )}
            {responseMessageTokenUtility.error && (
              <Alert className="ModalAlert" variant="danger">
                {responseMessageTokenUtility.error}
              </Alert>
            )}
        ...
        <Form className="modalForm" onSubmit={handleSubmitTokenDistribution}>
          ...
          <div>
            ...
            {responseMessageTokenDistri.success && (
              <Alert className="ModalAlert" variant="success">
                {responseMessageTokenDistri.success}
              </Alert>
            )}
            {responseMessageTokenDistri.error && (
              <Alert className="ModalAlert" variant="danger">
                {responseMessageTokenDistri.error}
              </Alert>
            )}
        ...
        <Form
          className="modalForm"
          onSubmit={handleSubmitValueAccrualMechanisms}
        >
          ...
          <div>
            ...
            {responseMessageValueAccrual.success && (
              <Alert className="ModalAlert" variant="success">
                {responseMessageValueAccrual.success}
              </Alert>
            )}
            {responseMessageValueAccrual.error && (
              <Alert className="ModalAlert" variant="danger">
                {responseMessageValueAccrual.error}
              </Alert>
            )}
